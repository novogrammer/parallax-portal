...
import { PortalRuntime } from '../dist/index.js'
import { PortalInstance } from '../dist/PortalInstance.js'

const projection = { referenceFovY: THREE.MathUtils.degToRad(42) }
...
test.before(() => {
  globalThis.window = { matchMedia: () => { throw new Error('unexpected responsive query') } }
})
...
test('camera clipping planes reject invalid portal configuration', () => {
  ...
  assert.throws(() => createRuntime({ cameraNear: 10, cameraFar: 10 }), /cameraFar/)
})
