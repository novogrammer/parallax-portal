...
const profile = { referenceFovY: 42 * Math.PI / 180 }
const scene = { cameraTopY: 3, cameraBottomY: 0 }
...
test('reference camera distance follows the projection equation', () => {
  ...
  assert.ok(Math.abs(calculateReferenceCameraDistance(profile, referenceProjectionHeightMeters) - expected) < 1e-12)
})

test('render FOV handles equal and unequal travel/reference heights', () => {
  const equalHeightFov = calculateRenderCameraFovY(profile, scene, referenceProjectionHeightMeters, 800, 400)
...
test('portal intersection covers partial, oversized, and invisible rects', () => {
  ...
  assert.equal(calculatePortalIntersection({ x: 1300, y: 0, width: 100, height: 100 }, viewport), null)
})
...
test('invalid projection inputs throw configuration errors', () => {
  ...
  assert.throws(() => calculateRenderCameraFovY(profile, scene, referenceProjectionHeightMeters, 0, 100), RangeError)
  assert.throws(() => calculateRenderCameraFovY(profile, scene, referenceProjectionHeightMeters, 100, 0), RangeError)
})
